...
all.arm64: $(ALL_IMAGES:=.amd64)

%.reaper.push.arm64: IMAGE ?= ${IMAGE_REPOSITORY}/reaper-plugin:${VERSION}-arm64-$*
%.reaper.push.arm64: %.reaper.image.arm64
	@docker push ${IMAGE}

%.reaper.image.arm64: IMAGE ?= ${IMAGE_REPOSITORY}/reaper-plugin:${VERSION}-arm64-$*
%.reaper.image.arm64:
	...
	@docker build -f docker/dist/arm64/reaper-plugin-$*.Dockerfile --tag ${IMAGE} .

%.reaper.push.amd64: IMAGE ?= ${IMAGE_REPOSITORY}/reaper-plugin:${VERSION}-amd64-$*
%.reaper.push.amd64: %.reaper.image.amd64
	@docker push ${IMAGE}

%.reaper.image.amd64: IMAGE ?= ${IMAGE_REPOSITORY}/reaper-plugin:${VERSION}-amd64-$*
%.reaper.image.amd64:
	...
	@docker build -f docker/dist/amd64/reaper-plugin-$*.Dockerfile --tag ${IMAGE} .

%.push.amd64: IMAGE ?= ${IMAGE_REPOSITORY}/$*:${VERSION}-amd64
%.push.amd64: %.image.amd64
	docker push ${IMAGE}

%.image.amd64: IMAGE ?= ${IMAGE_REPOSITORY}/$*:${VERSION}-amd64
%.image.amd64:
	@mkdir -p docker/dist/amd64
	...
	@docker build -f docker/dist/amd64/$*.Dockerfile --tag ${IMAGE} .

%.push.arm64: IMAGE ?= ${IMAGE_REPOSITORY}/$*:${VERSION}-amd64
%.push.arm64: %.image.amd64
	docker push ${IMAGE}

%.image.arm64: IMAGE ?= ${IMAGE_REPOSITORY}/$*:${VERSION}-arm64
%.image.arm64:
	@mkdir -p docker/dist/arm64
	...
	@docker build -f docker/dist/arm64/$*.Dockerfile --tag ${IMAGE} .
