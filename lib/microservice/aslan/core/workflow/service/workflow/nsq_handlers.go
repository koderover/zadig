...
type TaskAckHandler struct {
	...
	PoetryClient         *poetry.PoetryClient
	messages             chan *nsq.Message
	log                  *xlog.Logger
}

func NewTaskAckHandler(poetryServer, poetryRootKey string, maxInFlight int, log *xlog.Logger) *TaskAckHandler {
	return &TaskAckHandler{
		...
		PoetryClient:         poetry.NewPoetryServer(poetryServer, poetryRootKey),
				...
				if taskStatus == config.StatusPassed {
					...
					for _, subTask := range subBuildTaskMap {
						...
						if buildInfo.JobCtx.FileArchiveCtx != nil { // file
							...
							storageInfo, _ := s3.NewS3StorageFromEncryptedUri(pt.StorageUri, crypto.S3key)
				...
				if taskStatus == config.StatusPassed {
					...
					for _, subTask := range subDistributeFileTaskMap {
						...
						if artifactID != "" {
							...
							storageInfo, _ := s3.NewS3StorageFromEncryptedUri(releaseFileInfo.DestStorageUrl, crypto.S3key)
...
func (h *TaskAckHandler) createVersion(pt *task.Task) error {
	if pt.Status == config.StatusPassed && pt.Type == config.WorkflowType {
		if pt.WorkflowArgs != nil && pt.WorkflowArgs.VersionArgs != nil && pt.WorkflowArgs.VersionArgs.Enabled {
			...
			if isDeploy {
				//版本交付
				body, _ := json.Marshal(pt)
				uri := fmt.Sprintf("/aslan/delivery/releases?orgId=1&productName=%s&workflowName=%s&taskId=%d", pt.ProductName, pt.PipelineName, pt.TaskID)
				if _, err := h.PoetryClient.Do(uri, "POST", bytes.NewBuffer(body), h.PoetryClient.GetRootTokenHeader()); err != nil {
					return err
				}
			}
		}
...
func upsertWorkflowStat(args *commonmodels.WorkflowStat, log *xlog.Logger) error {
...
func getImageInfo(repoName, tag string, log *xlog.Logger) (*commonmodels.DeliveryImage, error) {
