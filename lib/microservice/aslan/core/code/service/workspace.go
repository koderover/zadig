...
func CleanWorkspace(username, pipelineName string, log *xlog.Logger) error {
...
func GetWorkspaceFilePath(username, pipelineName, file string, log *xlog.Logger) (string, error) {
...
func GetGitRepoInfo(codehostID int, repoOwner, repoName, branchName, remoteName, dir string, log *xlog.Logger) ([]*FileInfo, error) {
	fis := make([]*FileInfo, 0)
	if dir == "" {
		dir = "/"
	}
...
func GetPublicGitRepoInfo(urlPath, dir string, log *xlog.Logger) ([]*FileInfo, error) {
	fis := make([]*FileInfo, 0)
...
func GetGithubRepoInfo(codehostID int, repoName, branchName, path string, log *xlog.Logger) ([]*CodehostFileInfo, error) {
...
// 获取gitlab的目录内容接口
func GetGitlabRepoInfo(codehostID int, repoName, branchName, path string, log *xlog.Logger) ([]*CodehostFileInfo, error) {
