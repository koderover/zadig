...
import (
	...
	"github.com/koderover/zadig/lib/tool/helmclient"
...
// Init ...
func (p *DeployTaskPlugin) Init(jobname, filename string, xl *xlog.Logger) {
	p.JobName = jobname
	// SetLogger ...
	p.Log = xl
}
...
func (p *DeployTaskPlugin) getProductInfo(ctx context.Context, args *EnvArgs) (*types.Product, error) {
	...
	url := fmt.Sprintf("%s/environment/environments/%s/productInfo?envName=%s", config.AslanAddr(), args.ProductName, args.EnvName)
	body, err := client.Do("GET", url, getRootAuthHeader(), nil)
	if err != nil {
		return nil, err
	}
	if err := json.Unmarshal(body, &product); err != nil {
		p.Log.Errorf("json Unmarshal err :%v", err)
		return nil, err
	}
...
func (p *DeployTaskPlugin) getService(ctx context.Context, name, serviceType, productName string) (*types.ServiceTmpl, error) {
	...
	serviceObj := new(types.ServiceTmpl)
	url := fmt.Sprintf("%s/service/services/%s/%s?productName=%s", config.AslanAddr(), name, serviceType, productName)
	body, err := client.Do("GET", url, getRootAuthHeader(), nil)
	if err != nil {
		return nil, err
	}
	if err := json.Unmarshal(body, &serviceObj); err != nil {
		p.Log.Errorf("json Unmarshal err :%v", err)
		return nil, err
	}
...
func (p *DeployTaskPlugin) downloadService(pipelineTask *task.Task, serviceName, repoName string) error {
	...
	if s3Storage, err = s3.NewS3StorageFromEncryptedUri(pipelineTask.StorageUri, crypto.S3key); err != nil {
		return err
	}
...
func (p *DeployTaskPlugin) getRenderSet(ctx context.Context, name string, revision int64) (*types.RenderSet, error) {
	...
	renderSet := new(types.RenderSet)
	url := fmt.Sprintf("%s/project/renders/render/%s/revision/%d", config.AslanAddr(), name, revision)
	body, err := client.Do("GET", url, getRootAuthHeader(), nil)
	if err != nil {
		return nil, err
	}
	if err := json.Unmarshal(body, &renderSet); err != nil {
		p.Log.Errorf("json Unmarshal err :%v", err)
		return nil, err
	}
...
func (p *DeployTaskPlugin) updateRenderSet(ctx context.Context, args *types.RenderSet) error {
	...
	url := fmt.Sprintf("%s/project/renders", config.AslanAddr())
	body, _ := json.Marshal(args)
	if _, err := client.Do("PUT", url, getRootAuthHeader(), body); err != nil {
		p.Log.Errorf("updateRenderSet err:%+v", err)
		return err
	}
	return nil
}
