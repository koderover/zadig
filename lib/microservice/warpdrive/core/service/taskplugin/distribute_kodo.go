...
func (p *Distribute2S3TaskPlugin) Run(ctx context.Context, pipelineTask *task.Task, pipelineCtx *task.PipelineCtx, serviceName string) {
	...
	if _, err = os.Stat(localFile); os.IsNotExist(err) {
		...
		srcStorage, err = s3.NewS3StorageFromEncryptedUri(pipelineTask.StorageUri, config.S3key)
		if err != nil {
	...
	destStorage, err = s3.NewS3StorageFromEncryptedUri(p.Task.DestStorageUrl, config.S3key)
	if err != nil {
