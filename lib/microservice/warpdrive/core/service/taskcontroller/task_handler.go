...
// ExecHandler ...
// Sender: sender to send ack/notificaiton
...
func (h *ExecHandler) runPipelineTask(ctx context.Context, cancel context.CancelFunc, xl *xlog.Logger) {
	defer func() {
		h.SendNotification()
...
func (h *ExecHandler) execute(ctx context.Context, pipelineTask *task.Task, pipelineCtx *task.PipelineCtx, xl *xlog.Logger) {
...
// generateShortBsonId create an unique id which max length == 21
...
func (h *ExecHandler) executeTask(taskCtx context.Context, plugin plugins.TaskPlugin, subTask map[string]interface{}, pos int, servicename string, xl *xlog.Logger) (config.Status, error) {
...
func Logger(pipelineTask *task.Task) *xlog.Logger {
	// 初始化Logger
	var reqID string
	if pipelineTask == nil {
		reqID = xlog.GenReqID()
	} else {
		if pipelineTask.ReqID != "" {
			reqID = pipelineTask.ReqID
		} else {
			reqID = xlog.GenReqID()
		}
	}
