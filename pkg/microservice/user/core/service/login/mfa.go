...
type MFARecoveryCodesArgs struct {
	OTPCode      string `json:"otp_code,omitempty"`
	RecoveryCode string `json:"recovery_code,omitempty"`
}

type MFARecoveryCodesResp struct {
...
type UserMFAStatus struct {
	...
	SystemForcedEnabled bool   `json:"system_forced_enabled"`
}
...
func RegenerateRecoveryCodes(uid string, args *MFARecoveryCodesArgs, logger *zap.SugaredLogger) (*MFARecoveryCodesResp, error) {
	...
	if args == nil {
		return nil, fmt.Errorf("recovery code args are required")
	}
	if args.OTPCode == "" && args.RecoveryCode == "" {
		return nil, fmt.Errorf("otp code or recovery code is required")
	}
	...
	if err := orm.UpsertUserMFA(uid, &models.UserMFA{
		Model: models.Model{
		...
		RecoveryCodesJSON: string(recoveryCodesJSON),
	}, repository.DB); err != nil {
		return nil, err
	}
	if cacheErr := setUserMFAEnabledCache(uid, true); cacheErr != nil && logger != nil {
		logger.Warnf("failed to sync user mfa cache during recovery code regenerate, uid: %s, err: %v", uid, cacheErr)
	}

	return &MFARecoveryCodesResp{RecoveryCodes: recoveryCodes}, nil
}
...
func GetUserMFAStatus(uid string, logger *zap.SugaredLogger) (*UserMFAStatus, error) {
	...
	settings, err := commonrepo.NewSystemSettingColl().Get()
	if err != nil {
		return nil, err
	}

	systemforcedEnabled := false
	if settings.Security != nil {
		systemforcedEnabled = settings.Security.MFAEnabled
	}

	return &UserMFAStatus{
		UID:                 uid,
		Enabled:             userMFA != nil && userMFA.Enabled,
		SystemForcedEnabled: systemforcedEnabled,
	}, nil
}
