...
func (*Router) Inject(router *gin.RouterGroup) {
	users := router.Group("/users")
	{
		...
		users.POST("/:uid/mfa/setup", user.SetupUserMFA)
		users.POST("/:uid/mfa/enable", user.EnableUserMFA)
		users.POST("/:uid/mfa/disable", user.DisableUserMFA)
		users.POST("/:uid/mfa/recovery-codes", user.RegenerateRecoveryCodes)
		users.GET("/:uid/mfa/status", user.GetUserMFAStatus)
		users.DELETE("/:uid/mfa", user.ResetUserMFAByAdmin)
	...
	general := router.Group("")
	{
		...
		general.POST("/login/mfa/setup", login.MFASetup)
		general.POST("/login/mfa/enroll", login.MFAEnroll)
		general.POST("/login/mfa/verify", login.MFAVerify)
