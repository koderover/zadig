...
func NewGitlabOauth(redirectURI, clientID, clientSecret, hostName string) oauth.CallbackOauth {
	return &Oauth{
...
func (g *Oauth) oauth2Config() *oauth2.Config {
	...
	if g.HostName != "" {
		endpoint = oauth2.Endpoint{
			AuthURL:  g.HostName + "/oauth/authorize",
			TokenURL: g.HostName + "/oauth/token",
		}
	}
