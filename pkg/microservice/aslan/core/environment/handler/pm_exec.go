...
func ConnectSshPmExec(c *gin.Context) {
	...
	ctx.RespErr = service.ConnectSshPmExecWithIdentity(c, ctx.UserName, ctx.UserID, ctx.Account, name, projectKey, c.Param("serviceName"), ip, hostId, cols, rows, ctx.Logger)
}
