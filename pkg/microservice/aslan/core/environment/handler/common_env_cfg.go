...
func DeleteCommonEnvCfg(c *gin.Context) {
	...
	internalhandler.InsertDetailedOperationLog(c, ctx.UserName, projectKey, setting.OperationSceneEnv, "删除", "环境配置", fmt.Sprintf("%s:%s:%s", envName, commonEnvCfgType, objectName), fmt.Sprintf("%s:%s:%s", envName, commonEnvCfgType, objectName), "", types.RequestBodyTypeJSON, ctx.Logger, envName)
...
func CreateCommonEnvCfg(c *gin.Context) {
	...
	internalhandler.InsertDetailedOperationLog(c, ctx.UserName, c.Query("projectName"), setting.OperationSceneEnv, "新建", "环境配置", fmt.Sprintf("%s:%s", args.EnvName, args.CommonEnvCfgType), fmt.Sprintf("%s:%s", args.EnvName, args.CommonEnvCfgType), string(data), types.RequestBodyTypeJSON, ctx.Logger, c.Param("name"))
...
func UpdateCommonEnvCfg(c *gin.Context) {
	...
	internalhandler.InsertDetailedOperationLog(c, ctx.UserName, c.Query("projectName"), setting.OperationSceneEnv, "更新", "环境配置", fmt.Sprintf("%s:%s:%s", args.EnvName, args.CommonEnvCfgType, args.Name), fmt.Sprintf("%s:%s:%s", args.EnvName, args.CommonEnvCfgType, args.Name), string(data), types.RequestBodyTypeJSON, ctx.Logger, c.Param("name"))
	c.Request.Body = io.NopCloser(bytes.NewBuffer(data))
