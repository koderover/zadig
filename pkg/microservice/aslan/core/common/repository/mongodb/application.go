...
func (c *ApplicationColl) EnsureIndex(ctx context.Context) error {
	...
	_, err := c.Indexes().CreateMany(ctx, idxes, mongotool.CreateIndexOptions(ctx))
	return err
}
...
func (c *ApplicationColl) CreateCustomFieldUniqueIndex(ctx context.Context, key string) error {
	...
	_, err := c.Indexes().CreateOne(ctx, idx, mongotool.CreateIndexOptions(ctx))
...
func (c *ApplicationFieldDefinitionColl) EnsureIndex(ctx context.Context) error {
	_, err := c.Indexes().CreateMany(ctx, []mongo.IndexModel{{Keys: bson.D{{Key: "key", Value: 1}}, Options: options.Index().SetUnique(true)}}, mongotool.CreateIndexOptions(ctx))
	return err
}
