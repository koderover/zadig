...
func (m *JobLogManager) GetJobKey(projectName, workflowName string, taskID int64, jobTaskName string) string {
	return fmt.Sprintf("job-%s-%s-%d-%s", projectName, workflowName, taskID, jobTaskName)
}

func (m *JobLogManager) GetJobLogKey(projectName, workflowName string, taskID int64, jobTaskName string) string {
	return fmt.Sprintf("job-log-%s-%s-%d-%s", projectName, workflowName, taskID, jobTaskName)
}

func (m *JobLogManager) SaveJobLog(workflowCtx *commonmodels.WorkflowTaskCtx, jobTask *commonmodels.JobTask, logContent string) (err error) {
	logKey := m.GetJobLogKey(workflowCtx.ProjectName, workflowCtx.WorkflowName, workflowCtx.TaskID, jobTask.Name)
	...
	if logContent != "" {
		...
		if err != nil {
			err = fmt.Errorf("failed to write job log to redis, project: %s, workflow: %s, taskID: %d, error: %s", workflowCtx.ProjectName, workflowCtx.WorkflowName, workflowCtx.TaskID, err)
...
// func (m *JobLogManager) jobFinished(status config.Status) bool {
// 	return status == config.StatusCancelled || status == config.StatusTimeout || status == config.StatusFailed || status == config.StatusPassed
// }

func (m *JobLogManager) uploadJobLogToS3(workflowCtx *commonmodels.WorkflowTaskCtx, jobTask *commonmodels.JobTask) error {
	...
	if store.Subfolder != "" {
		store.Subfolder = fmt.Sprintf("%s/%s/%d/%s", store.Subfolder, strings.ToLower(workflowCtx.WorkflowName), workflowCtx.TaskID, "log")
	} else {
		store.Subfolder = fmt.Sprintf("%s/%d/%s", strings.ToLower(workflowCtx.WorkflowName), workflowCtx.TaskID, "log")
	}
	...
	if err != nil {
		return fmt.Errorf("failed to get job log from redis, project: %s, workflow: %s, taskID: %d error: %s", workflowCtx.ProjectName, workflowCtx.WorkflowName, workflowCtx.TaskID, err)
	}
	...
	fileName := strings.Replace(strings.ToLower(jobTask.Name), "_", "-", -1)
	...
	log.Infof("uploadJobLogToS3 s3 upload success, workflowName:%s jobTaskKey:%s, taskID:%d", workflowCtx.WorkflowName, jobTask.Name, workflowCtx.TaskID)
...
func (m *JobLogManager) FinishJob(workflowCtx *commonmodels.WorkflowTaskCtx, jobTask *commonmodels.JobTask) {
	jobKey := m.GetJobKey(workflowCtx.ProjectName, workflowCtx.WorkflowName, workflowCtx.TaskID, jobTask.Name)
	cache.NewRedisCache(pkgconfig.RedisCommonCacheTokenDB()).Delete(jobKey)

	jobLogKey := m.GetJobLogKey(workflowCtx.ProjectName, workflowCtx.WorkflowName, workflowCtx.TaskID, jobTask.Name)

	// after the task execution ends, synchronize the logs in the file to s3
	err := m.uploadJobLogToS3(workflowCtx, jobTask)
	if err != nil {
		err = fmt.Errorf("failed to upload job log to s3, project: %s, workflow: %s, taskID: %d, error: %s", workflowCtx.ProjectName, workflowCtx.WorkflowName, workflowCtx.TaskID, err)
		log.Errorf(err.Error())
	}
