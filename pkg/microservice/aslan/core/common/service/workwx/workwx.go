...
func GetLarkClientByIMAppID(id string) (*workwx.Client, error) {
	approval, err := mongodb.NewIMAppColl().GetByID(context.Background(), id)
	if err != nil {
		return nil, errors.Wrap(err, "get external approval data")
	}
	if approval.Type != setting.IMWorkWx {
		return nil, errors.Errorf("unexpected approval type %s", approval.Type)
	}
	return workwx.NewClient(approval.Host, approval.CorpID, approval.AgentID, approval.AgentSecret), nil
}
