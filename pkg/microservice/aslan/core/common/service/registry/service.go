...
func (c *authClient) validateRegistry() (err error) {
	...
	count, err := reg.Repositories(context.Background(), repos, "")
	if err != nil {
		return errors.Wrapf(err, "failed to list repositories with Docker Distribution client")
	}

	c.log.Debugf("Docker Distribution client successfully listed %d repositories", count)
	return nil
}
...
func (c *authClient) validateRegistryHttp() error {
	...
	// 3. 使用 token 访问 catalog 端点
	catalogURL := strings.TrimSuffix(c.endpoint.Addr, "/") + "/v2/_catalog"
	...
	if catalogResp.StatusCode == http.StatusOK {
		c.log.Info("Registry validation successful via catalog access with token")
		return nil
	}

	return errors.New("catalog access failed with status " + strconv.Itoa(catalogResp.StatusCode))
}
...
func (c *authClient) getAuthToken(authHeader string) (string, error) {
	...
	// 构建 token 请求 URL
	tokenURL := fmt.Sprintf("%s?service=%s&scope=registry:catalog:*", realm, service)
