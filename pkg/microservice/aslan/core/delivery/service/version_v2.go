...
func CreateK8SDeliveryVersionV2(args *CreateDeliveryVersionRequest, logger *zap.SugaredLogger) error {
	...
	if len(args.ImageRegistryID) != 0 {
		...
		if !targetRegistryProjectSet.Has(args.ProjectName) && !targetRegistryProjectSet.Has(setting.AllProjects) {
			return fmt.Errorf("registry %s/%s not support project %s", targetRegistry.RegAddr, targetRegistry.Namespace, args.ProjectName)
		}
	}
	...
	err = commonrepo.NewDeliveryVersionV2Coll().Insert(versionObj)
	if err != nil {
...
func getImageSourceRegistryV2(imageData *commonmodels.DeliveryVersionImage, registryMap map[string]*commonmodels.RegistryNamespace) (*commonmodels.RegistryNamespace, error) {
	sourceImageTag := ""
	registryURL := strings.TrimSuffix(imageData.TargetImage, fmt.Sprintf("/%s", imageData.ImageName))
	tmpArr := strings.Split(imageData.TargetImage, ":")
	if len(tmpArr) == 2 {
		sourceImageTag = tmpArr[1]
		registryURL = strings.TrimSuffix(imageData.TargetImage, fmt.Sprintf("/%s:%s", imageData.ImageName, sourceImageTag))
		...
		return nil, fmt.Errorf("invalid image: %s", imageData.TargetImage)
	}
	sourceRegistry, ok := registryMap[registryURL]
	if !ok {
		return nil, fmt.Errorf("can't find source registry for image: %s", imageData.TargetImage)
	}
...
func buildDeliveryImagesV2(targetRegistry *commonmodels.RegistryNamespace, registryMap map[string]*commonmodels.RegistryNamespace, deliveryVersion *commonmodels.DeliveryVersionV2, logger *zap.SugaredLogger) (err error) {
	...
	if len(deliveryVersionWorkflowV4.Stages) != 0 {
		...
		err = commonrepo.NewDeliveryVersionV2Coll().UpdateWorkflowTask(deliveryVersion.Version, deliveryVersion.ProjectName, deliveryVersion.WorkflowName, int32(deliveryVersion.TaskID))
		if err != nil {
			logger.Errorf("failed to update delivery version task_id, version: %s, task_id: %s, err: %s", deliveryVersion, deliveryVersion.ProjectName, deliveryVersion.TaskID)
		}
	}
...
func RetryCreateK8SDeliveryVersionV2(projectName, versionName string, logger *zap.SugaredLogger) error {
	deliveryVersion, err := commonrepo.NewDeliveryVersionV2Coll().Get(&commonrepo.DeliveryVersionV2Args{
		ProjectName: projectName,
		Version:     versionName,
	})
	if err != nil {
...
func RetryDeliveryVersionV2(projectName, versionName string, versionType setting.DeliveryVersionType, logger *zap.SugaredLogger) error {
	if versionType == setting.DeliveryVersionTypeChart {
		return RetryCreateHelmDeliveryVersionV2(projectName, versionName, logger)
	} else if versionType == setting.DeliveryVersionTypeYaml {
		return RetryCreateK8SDeliveryVersionV2(projectName, versionName, logger)
	}

	return fmt.Errorf("invalid version type: %s", versionType)
}

func RetryCreateHelmDeliveryVersionV2(projectName, versionName string, logger *zap.SugaredLogger) error {
	deliveryVersion, err := commonrepo.NewDeliveryVersionV2Coll().Get(&commonrepo.DeliveryVersionV2Args{
		ProjectName: projectName,
		Version:     versionName,
	})
	...
	err = buildDeliveryChartsV2(deliveryVersion, logger)
	if err != nil {
		return err
	}
...
func CreateHelmDeliveryVersionV2(args *CreateDeliveryVersionRequest, logger *zap.SugaredLogger) error {
	...
	err = buildDeliveryChartsV2(versionObj, logger)
	if err != nil {
		return err
	}
...
func downloadChartV2(deliveryVersion *commonmodels.DeliveryVersionV2, service *commonmodels.DeliveryVersionService) (string, error) {
	productName, versionName := deliveryVersion.ProjectName, deliveryVersion.Version
	chartTGZName := fmt.Sprintf("%s-%s.tgz", service.ServiceName, service.ChartVersion)
	...
	chartRepo, err := getChartRepoData(deliveryVersion.ChartRepoName)
	if err != nil {
		return "", err
	}
	...
	chartRef := fmt.Sprintf("%s/%s", chartRepo.RepoName, service.ServiceName)
	return chartTGZFilePath, hClient.DownloadChart(commonutil.GeneHelmRepo(chartRepo), chartRef, service.ChartVersion, chartTGZFileParent, false)
}

func buildDeliveryChartsV2(deliveryVersion *commonmodels.DeliveryVersionV2, logger *zap.SugaredLogger) (err error) {
	defer func() {
		if err != nil {
			deliveryVersion.Status = setting.DeliveryVersionStatusFailed
			deliveryVersion.Error = err.Error()
	...
	if (deliveryVersion.Source == setting.DeliveryVersionSourceFromEnv ||
		deliveryVersion.Source == setting.DeliveryVersionSourceFromVersion) &&
		deliveryVersion.EnvName != "" {

		dir, err2 := makeChartTGZFileDir(deliveryVersion.ProjectName, deliveryVersion.Version)
		if err2 != nil {
			return err2
		}
		repoInfo, err2 := getChartRepoData(deliveryVersion.ChartRepoName)
		if err2 != nil {
			log.Errorf("failed to query chart-repo info, productName: %s, err: %s", deliveryVersion.ProjectName, err)
			return fmt.Errorf("failed to query chart-repo info, productName: %s, repoName: %s", deliveryVersion.ProjectName, deliveryVersion.ChartRepoName)
		}

		env, err2 := commonrepo.NewProductColl().Find(&commonrepo.ProductFindOptions{
		...
		for _, service := range deliveryVersion.Services {
			...
			go func(service *commonmodels.DeliveryVersionService) {
				defer wg.Done()
				err := pushDeliveryChart(service, env, repoInfo, dir)
				if err != nil {
	...
	var targetRegistry *commonmodels.RegistryNamespace
	for _, registry := range registryMap {
		if registry.ID.Hex() == deliveryVersion.ImageRegistryID {
			targetRegistry = registry
			break
		}
	}
...
func pushDeliveryChart(service *commonmodels.DeliveryVersionService, env *commonmodels.Product, chartRepo *commonmodels.HelmRepo, dir string) error {
	deliveryChartPath, err := ensureChartFilesV2(service, env)
	if err != nil {
		return err
	}
	...
	err = os.WriteFile(valuesFilePath, []byte(service.YamlContent), 0644)
	if err != nil {
	...
	//set metadata
	chartRequested.Metadata.Name = service.ServiceName
...
func ensureChartFilesV2(service *commonmodels.DeliveryVersionService, env *commonmodels.Product) (string, error) {
...
func GetDeliveryVersionDetailV2(args *commonrepo.DeliveryVersionV2Args, log *zap.SugaredLogger) (*commonmodels.DeliveryVersionV2, error) {
	...
	// if version.Status == setting.DeliveryVersionStatusSuccess || version.Status == setting.DeliveryVersionStatusFailed {
	// 	return version, nil
	// } else {
	// 	if version.WorkflowName != "" && version.TaskID != 0 {
	// 		// calc image distribute status
	// 		err = checkDeliveryImageStatus(version, log)
	// 		if err != nil {
	// 			return nil, fmt.Errorf("failed to check delivery image status, err: %v", err)
	// 		}
	// 	}
	// }
...
func checkDeliveryImageStatus(version *commonmodels.DeliveryVersionV2, workflowTask *commonmodels.WorkflowTask, log *zap.SugaredLogger) (bool, error) {
	...
	for _, service := range version.Services {
		for _, image := range service.Images {
			...
			if image.Status != distributeStatusMap[key].status {
				image.Status = distributeStatusMap[key].status
				image.Error = distributeStatusMap[key].err.Error()
				changed = true
			}
		}
	}
...
func preDownloadChart(projectName, versionName, chartName string, log *zap.SugaredLogger) (string, error) {
	...
	// prepare chart data
	filePath, err := downloadChartV2(deliveryInfo, service)
	if err != nil {
		return "", err
	}
