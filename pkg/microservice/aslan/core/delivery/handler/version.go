...
// @Param 	type			query		setting.DeliveryVersionType 	true	"type"
// @Success 200
...
func RetryDeliveryVersion(c *gin.Context) {
	...
	if version == "" {
		ctx.RespErr = e.ErrInvalidParam.AddDesc("version can't be empty!")
		return
	}
	...
	ctx.RespErr = deliveryservice.RetryDeliveryVersionV2(projectName, version, setting.DeliveryVersionType(versionType), ctx.Logger)
}
