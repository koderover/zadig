...
type pipelineGitEventMatcher interface {
	Match(*commonmodels.MainHookRepo, string) (*commonmodels.TaskArgs, error)
}
...
func (pgpem *pipelineGitlabPushEventMatcher) Match(hookRepo *commonmodels.MainHookRepo, requestID string) (*commonmodels.TaskArgs, error) {
	ev := pgpem.event
	var (
...
func (pgmem *pipelineGitlabMergeEventMatcher) Match(hookRepo *commonmodels.MainHookRepo, requestID string) (*commonmodels.TaskArgs, error) {
	ev := pgmem.event
	var (
		err          error
		changedFiles = make([]string, 0)
...
func pipelineCreateGitlabEventMatcher(
	event interface{}, diffSrv gitlabMergeRequestDiffFunc, pipeline *commonmodels.Pipeline, log *zap.SugaredLogger,
	...
	for _, pipelineObject := range pipelineList {
		if pipelineObject.Hook != nil && pipelineObject.Hook.Enabled {
			...
			for _, item := range pipelineObject.Hook.GitHooks {
				...
				taskargs, err := matcher.Match(hookRepo, requestID)
