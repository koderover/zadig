...
func validateWorkflowEnvironmentJobs(jobs []*commonmodels.Job, permission *workflowEnvironmentPermission) error {
	if permission == nil {
		return nil
	}
	for _, job := range jobs {
		if job == nil || job.Skipped || !isEnvironmentJob(job.JobType) {
			continue
		}
		spec, err := workflowJobSpec(job)
		if err != nil {
		...
		environment := environmentFromJobSpec(job.JobType, spec)
		if environment != "" && !permission.isAllowed(environment, production) {
			return fmt.Errorf("workflow task execution denied: job %s cannot use environment %s without environment permission", job.Name, environment)
		}
	}
	return nil
}
...
func validateWorkflowEnvironmentSelection(workflow *commonmodels.WorkflowV4, permission *workflowEnvironmentPermission) error {
	...
	for _, stage := range workflow.Stages {
		...
		if err := validateWorkflowEnvironmentJobs(stage.Jobs, permission); err != nil {
			return err
		}
	}
	return nil
