...
func (j PingCodeJobController) Validate(isExecution bool) error {
	...
	if err := commonmodels.IToi(currJob.Spec, currJobSpec); err != nil {
		return fmt.Errorf("failed to decode pingcode job spec, error: %s", err)
	}
...
func (j PingCodeJobController) ToTask(taskID int64) ([]*commonmodels.JobTask, error) {
	...
	info, err := mongodb.NewProjectManagementColl().GetPingCodeByID(j.jobSpec.PingCodeID)
	if err != nil {
	...
	jobTask := &commonmodels.JobTask{
		...
		Spec: commonmodels.JobTaskPingCodeSpec{
			PingCodeAddress:      info.PingCodeAddress,
			PingCodeClientID:     info.PingCodeClientID,
			PingCodeClientSecret: info.PingCodeClientSecret,
