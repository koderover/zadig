	...
	if j.spec.TestType == config.ServiceTestType {
		for _, target := range j.spec.TargetServices {
			for _, testing := range j.spec.ServiceAndTests {
				...
				logger.Errorf("@@@@ testing : %s", testing)
				// logger.Errorf("@@@@ testing.TestModule: %s", testing.TestModule.Name)
				logger.Errorf("@@@@ testing.: %s", testing.Name)
				jobTask, err := j.toJobtask(testing, defaultS3, taskID, string(j.spec.TestType), testing.ServiceName, testing.ServiceModule, logger)
				if err != nil {
					return resp, err
				}
				resp = append(resp, jobTask)
