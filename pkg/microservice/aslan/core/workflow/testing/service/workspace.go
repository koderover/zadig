...
type GetArtifactInfoResp struct {
	FileNames          []string `json:"file_names"`
	NotHistoryFileFlag bool     `json:"not_history_file_flag"`
}

func GetTestArtifactInfo(pipelineName, dir string, taskID int64, log *zap.SugaredLogger) (*GetArtifactInfoResp, error) {
	resp := new(GetArtifactInfoResp)
	fis := make([]string, 0)
...
func GetWorkflowV4ArtifactInfo(workflowName, jobName string, taskID int64, log *zap.SugaredLogger) (*GetArtifactInfoResp, error) {
	resp := new(GetArtifactInfoResp)
	...
	for _, stage := range workflowTask.Stages {
		for _, job := range stage.Jobs {
			...
			if job.JobType != string(config.JobZadigTesting) && job.JobType != string(config.JobZadigScanning) {
				return resp, fmt.Errorf("job: %s was not a testing or scanning job", jobName)
			}
			jobTask = job
		}
	}
...
func GetWorkflowV4ScanningArtifactInfo(scanningID, jobName string, taskID int64, log *zap.SugaredLogger) (*GetArtifactInfoResp, error) {
	resp := new(GetArtifactInfoResp)
	...
	for _, stage := range workflowTask.Stages {
		for _, job := range stage.Jobs {
			...
			if job.JobType != string(config.JobZadigScanning) {
				return resp, fmt.Errorf("job: %s was not a scanning job", jobName)
			}
			jobTask = job
		}
	}
	...
	for _, step := range jobSpec.Steps {
		...
		if step.StepType != config.StepTarArchive {
			return resp, fmt.Errorf("step: %s was not a archive scanning result step", step.Name)
		}
		stepTask = step
	}
	if stepTask == nil {
	...
	object, err := client.GetFile(storage.Bucket, objectKey, &s3tool.DownloadOption{RetryNum: 2})
	if err != nil {
		log.Errorf("GetWorkflowV4ScanningArtifactInfo GetFile err:%s", err)
		return resp, err
	}
	if object != nil && *object.ContentLength > 0 {
		tempDir, _ := ioutil.TempDir("", "")
	...
	files, err := client.ListFiles(storage.Bucket, prefix, true)
	if err != nil || len(files) <= 0 {
		log.Errorf("GetWorkflowV4ScanningArtifactInfo ListFiles err:%s", err)
		return resp, err
	}
