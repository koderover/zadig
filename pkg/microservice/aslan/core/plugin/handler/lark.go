...
func UpdateLarkStageServiceConfigV2(c *gin.Context) {
	...
	if err := validateUpdateLarkStageServiceConfigV2(req); err != nil {
		ctx.RespErr = err
		return
	}

	ctx.RespErr = service.UpdateLarkStageServiceConfigV2(ctx, stageName, workitemTypeKey, workItemID, req)
}

func validateUpdateLarkStageServiceConfigV2(req *service.UpdateLarkStageServiceConfigV2Req) error {
	seen := make(map[string]struct{})
	for _, cfg := range req.Configs {
		key := fmt.Sprintf("%d-%s-%s", cfg.TemplateID, cfg.ServiceName, cfg.ServiceModule)
		if _, exists := seen[key]; exists {
			return fmt.Errorf("duplicate service config: template_id=%d, service=%s/%s", cfg.TemplateID, cfg.ServiceName, cfg.ServiceModule)
		}
		seen[key] = struct{}{}
	}
	return nil
