...
// @router /api/plugin/lark/workitem/type/:workitemTypeKey/node [get]
func GetLarkWorkitemTypeNodes(c *gin.Context) {
	...
	ctx.Resp, ctx.RespErr = service.GetLarkWorkitemTypeNodes(ctx, workitemTypeKey)
}
...
func GetLarkWorkitemWorkflow(c *gin.Context) {
	...
	ctx.Resp, ctx.RespErr = service.GetLarkWorkitemWorkflow(ctx, workflowTypeKey, workItemID, authProjects, authWorkflows)
}
...
func ExecuteLarkWorkitemWorkflow(c *gin.Context) {
	...
	ctx.Resp, ctx.RespErr = service.ExecuteLarkWorkitemWorkflow(ctx, workItemTypeKey, workItemID, args)
}
...
func CheckLarkAuth(c *gin.Context, ctx *internalhandler.Context) error {
	token := c.GetHeader("X-PLUGIN-TOKEN")
	if token == "" {
		return fmt.Errorf("Unauthorized, missing X-PLUGIN-TOKEN")
	}

	userKey := c.GetHeader("X-USER-KEY")
	if userKey == "" {
		return fmt.Errorf("Unauthorized, missing X-USER-KEY")
	}
	...
	userTokenStr, err := cache.NewRedisCache(config.RedisCommonCacheTokenDB()).GetString(fmt.Sprintf("lark_plugin_user_token_%s_%s", workspaceID, userKey))
	if err != nil {
		if errors.Is(err, redis.Nil) {
			return fmt.Errorf("Unauthorized, user token not found")
		}
		return fmt.Errorf("Unauthorized, failed to get user token from cache: %w", err)
	}
	...
	if userToken.UserKey != userKey {
		return fmt.Errorf("Unauthorized, user key mismatch")
	}
