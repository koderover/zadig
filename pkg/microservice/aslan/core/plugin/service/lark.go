...
func GetLarkWorkitemTypeNodes(ctx *internalhandler.Context, workitemTypeKey string) (*GetLarkWorkitemTypeNodesResponse, error) {
	...
	for _, template := range templateMap {
		...
		larkResp2, err := client.ClientV2.WorkItem.QueryTemplateDetail(ctx, (*workitem.QueryTemplateDetailReq)(workitem.NewQueryTemplateDetailReqBuilder().
...
func GetLarkWorkitemWorkflow(ctx *internalhandler.Context, workItemType, workItemID string, isAdmin bool, authProjects, authWorkflows sets.Set[string]) (*GetLarkWorkitemWorkflowResponse, error) {
	...
	for _, node := range templateWorkflowConfig {
		...
		for _, workflowConfigNode := range workflowConfig.Nodes {
			log.Debugf("db workflow config node: %s", workflowConfigNode.ID)
			// 仅列出配置了工作流的节点
			if *node.StateKey == workflowConfigNode.ID {
				...
				NodeIDWorkflowMap[workflowConfigNode.ID] = &NodeWorkflows{
					Node: &Node{
				...
				NodeIDWorkflowNamesMap[workflowConfigNode.ID] = append(NodeIDWorkflowNamesMap[workflowConfigNode.ID], workflowConfigNode.WorkflowName)
			}
		}
	}
