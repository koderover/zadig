...
以及需求研发周期等数据;你需要从两个月数据中做对比，分析出两个月中变化最大的5条变化的内容，输出格式为json格式;下面用三个圆括号分割的内容是你需要返回的json数据格式的定义和字段说明:(((
{
  \"answer\": [
    {
      \"project\": 项目名称，示例：\"yaml-test-2\",
      \"result\": 分析结果，示例：\"项目yaml-test-2测试成功率提升了47.2%\",
      \"name\": 用于比较的参数名称，示例：\"test_success_rate\",
      \"current_month\": 本月的值，示例：\"77.20\",
      \"last_month\": 上个月的值：示例：\"30.00\"
    }
  ]
}
)));
你的返回值需要严格符合所定义的返回值结果；你可以按照以下步骤来进行分析数据：
1.步骤一，计算current_month和last_month的构建成功率，测试成功率，部署成功率，发布成功率，发布频次，以及需求研发周期等数据的差值,差值计算公式：current_month-last_month，你必须对计算结果做检查，判断是否正确，否则重新计算；
2.步骤二，根据差值的绝对值从大到小排序，取前三个差值最大的数据；
3.步骤三，根据差值的正负，分别输出三条变化的内容，如果差值为正，则输出“项目<project_name>测试成功率提升了40%”，如果差值为负，则输出“项目<project_name>测试成功率下降了40%”。
你的输出必须是JSON格式，根据两个月差异的从大到小进行排序再返回，保证差异最大的内容在最上面；返回前必须对你的回答进行检查，有错误则重新进行计算并分析；同时你的回答禁止出现提升-40%这种结果，如果差值是负数，你需要回答下降了，
