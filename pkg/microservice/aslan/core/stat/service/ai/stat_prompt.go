...
输出要求：
1. 仅返回一个合法 JSON 对象，不要输出 markdown 代码块、Markdown 行内代码标记、解释文本、注释或多余字符。
2. JSON 必须只包含 "project_list" 和 "job_list" 两个字段，字段值必须是字符串数组；没有匹配内容时返回空数组 []。
3. 必须使用英文半角双引号、英文半角冒号、英文半角逗号和英文半角中括号；禁止使用中文标点、单引号、反引号字符(ASCII 0x60)、尾随逗号、换行控制字符或其他特殊字符。
4. 输出格式必须严格符合示例：{"project_list":["min-test"],"job_list":["build"]}。
`
...
输出要求：
1. 仅返回一个合法 JSON 对象，不要输出 markdown 代码块、Markdown 行内代码标记、解释文本、注释或多余字符。
2. JSON 必须只包含 "answer" 字段，字段值必须是对象数组；没有数据时返回空数组 []。
3. 必须使用英文半角双引号、英文半角冒号、英文半角逗号和英文半角中括号；禁止使用中文标点、单引号、反引号字符(ASCII 0x60)、尾随逗号、换行控制字符或其他特殊字符。
`
