...
请按前面的规则提取下面 三重引号 内的内容，同时需要校验自己的答案是否符合提取规则和输出要求。
输出要求：仅返回JSON数据，不要有任何多余解释过程的输出。
你只能输出一个可被标准JSON解析器直接解析的JSON对象，禁止输出markdown代码块标记（如三个反引号代码块）、注释、说明文字或任何额外字符。
输出格式必须严格为{\"project_list\":[...],\"job_list\":[...]}，字段名固定且仅允许这两个字段。
硬性约束（必须全部满足）：1）返回内容只能包含ASCII字符；2）禁止输出任何非ASCII字符（例如å、中文字符、全角标点、不可见特殊符号）；3）禁止在JSON前后输出任何字符。
如果你无法保证上述约束，必须返回兜底JSON：{\"project_list\":[],\"job_list\":[]}。
所有键名和字符串值必须使用双引号；字符串中的特殊字符必须正确转义：双引号使用\\\"，反斜杠使用\\\\，换行使用\\n；禁止输出未转义的控制字符。
返回前必须自检：若结果不能被JSON反序列化则重新生成。
`
...
你的回答不要超过400个汉字。
输出约束：只能输出纯文本分析结果，禁止输出markdown代码块标记、JSON、XML、注释、前后缀分隔符和任何额外结构化包装。
硬性约束（必须全部满足）：1）返回内容只能包含ASCII字符；2）禁止输出任何非ASCII字符（例如å、全角标点、不可见特殊符号）；3）禁止输出控制字符。
如果无法满足上述约束，返回固定文本：analysis unavailable。`
...
你的回答需要使用text格式输出, 输出内容不要包含\"三重引号分割的项目数据\"这个名称，也不要复述分析要求中的内容,需要给出明确的分析结论,在你的回答中禁止包含 \"data_description\" 字段;
输出约束：只能输出纯文本分析结果，禁止输出markdown代码块标记、JSON、XML、注释、前后缀分隔符和任何额外结构化包装。
硬性约束（必须全部满足）：1）返回内容只能包含ASCII字符；2）禁止输出任何非ASCII字符（例如å、全角标点、不可见特殊符号）；3）禁止输出控制字符。
如果无法满足上述约束，返回固定文本：analysis unavailable。
项目数据：\"\"\"%s\"\"\";`
...
要求：你只能输出一个可被标准JSON解析器直接解析的JSON对象，禁止输出markdown代码块标记（如三个反引号开头的json代码块）、注释、说明文字、前后缀分隔符（如((()/)))）或任何额外字符。
输出格式必须严格为{\"answer\":[...]}；所有键名和字符串值必须使用双引号。
硬性约束（必须全部满足）：1）返回内容只能包含ASCII字符；2）禁止输出任何非ASCII字符（例如å、中文字符、全角标点、不可见特殊符号）；3）禁止在JSON前后输出任何字符。
如果你无法保证上述约束，必须返回兜底JSON：{\"answer\":[]}。
字符串中的特殊字符必须正确转义：双引号使用\\\"，反斜杠使用\\\\，换行使用\\n；禁止输出未转义的控制字符。
返回前必须自检：若结果不能被JSON反序列化则重新生成。
