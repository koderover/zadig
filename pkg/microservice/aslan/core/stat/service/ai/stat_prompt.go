...
文本提取器规则为下列1. 2. 3. 三条规则：
1. 开始和截止时间: 根据三重引号分割的输入数据解析开始时间戳和截止时间戳，注意，开始时间和截止时间都是10位的秒级时间戳，生成后对开始时间和截止时间做校验，无误才返回；
下列三重大括号内数据是解析开始时间和截止时间的用例。
{{{
> 用例一：
输入："""分析最近一个月ai-demo项目的构建效率"""
假设当前日期是2022年7月16日，那么最近一个月的开始时间戳和截止时间戳如下所示：
开始时间戳：1655364423 （对应的日期是2022年6月16日）
截止时间戳：1657956423 （对应的日期是2022年7月16日）
输出：{"start":1655364423,"end": 1657956423}

> 用例二：
输入："""最近这个季度项目整体水平"""
假设当前日期是2021年5月13日，那么最近这个季度的开始时间戳和截止时间戳如下所示：
开始时间戳：1613201223 （对应的日期是2021年2月13日）
截止时间戳：1620890823 （对应的日期是2021年5月13日）
}}}
2. 项目列表：返回三重引号分割的输入数据中的项目名称列表。
...
3. job列表：job列表是指项目的构建(build)、测试(test)、部署(deploy)、发布(release)这四个过程，请在三重引号分割的输入数据中匹配这四个过程，如果匹配到，则返回匹配到的job列表，
如果没有匹配到，则返回空数组。
...
const ProjectAnalysisPrompt = `假设你是资深Devops专家，你需要根据分析要求去分析三重引号分割的项目数据，分析要求：%s;你要在理解所有项目数据的前提下进行分析，
每个项目的各个数据你可以通过项目数据中的\"data_description\"字段来理解数据内容；如果存在多个项目，你需要在分析时候从构建、测试、部署、发布几个角度来进行对比并深度分析；
你的回答需要使用text格式输出, 输出内容不要包含\"三重引号分割的项目数据\"这个名称，也不要复述分析要求中的内容,同时不要包含项目数据中data_description等数据字段; 项目数据：\"\"\"%s\"\"\";`
...
const MonthAttentionAnalysisPrompt = `你是DevOps专家，你需要根据三重引号分割的输入数据，该数据分别包括了 %d月 和 %d月 每个月各自的构建成功率，测试成功率，部署成功率，发布成功率，发布频次，
以及需求研发周期等数据;你需要从两个月数据中做对比，分析出两个月中变化最大的5条变化的内容，输出格式为json格式，你需要根据三个括号分割的样例来输出一样格式的内容，内容来源于你的分析结果,样例:(((
{
  \"answer\": [
    {
      \"project\": \"项目A\",
...
你的输出必须是JSON格式，返回前必须对你的回答进行检查，并根据两个月差异的从大到小进行排序再返回，保证差异最大的内容在最上面，有错误则重新进行计算并分析,同时如果差值是负数，你需要回答下降了，
如果差值是正的，则回答提升了，如果项目数大于5，则返回变化最大的5条内容，如果项目数量小于5条，则返回变化最大的3条内容，如果没有数据，则返回如下JSON内容{\n    \"answer\":[]\n};
输入数据\"\""\"%s\"\"\"
`
