...
var PromptExamples = []string{
	"通过历史数据，总结%s项目最近一个月的整体表现。",
	"请根据项目%s的构建、部署、测试和发布等数据，分析项目最近一个月的现状，并基于历史数据，分析未来的趋势和潜在问题，并提出改进建议和措施。",
	"根据%s项目最近一个月每周的构建，部署，测试等数据的变化，以此分析该项目最近一段时间的发展趋势，如果存在问题则分析原因并给出合理的建议和解决方案。",
	"通过历史数据，分析%s项目的最大短板是什么？并针对这些短板提供一些解决办法和工具。",
...
你的输出必须是JSON格式，返回前必须对你的回答进行检查，并根据两个月差异的从大到小进行排序再返回，保证差异最大的内容在最上面，有错误则重新进行计算并分析,同时如果差值是负数，你需要回答下降了，如果差值是正的，则回答提升了，如果项目数大于5，则返回变化最大的5条内容，如果项目数量小于5条，则返回变化最大的3条内容，如果没有数据，则返回如下JSON内容{\n    \"answer\":[]\n}。
`
