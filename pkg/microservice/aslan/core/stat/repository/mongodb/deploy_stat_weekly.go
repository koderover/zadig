...
type WeeklyDeployStatColl struct {
	*mongo.Collection

	coll string
}

func NewWeeklyDeployStatColl() *WeeklyDeployStatColl {
	name := models.WeeklyDeployStat{}.TableName()
	return &WeeklyDeployStatColl{Collection: mongotool.Database(config.MongoDatabase()).Collection(name), coll: name}
}

func (c *WeeklyDeployStatColl) GetCollectionName() string {
	return c.coll
}
...
func (c *WeeklyDeployStatColl) Upsert(args *models.WeeklyDeployStat) error {
	if args == nil {
...
func (c *WeeklyDeployStatColl) CalculateStat(startTime, endTime int64, projects []string, productionType config.ProductionType) ([]*models.WeeklyDeployStat, error) {
	query := bson.M{
