...
func ListPingCodeWorkItems(id, projectID string, stateIDs, sprintIDs, boardIDs []string, keywords string) ([]*pingcode.WorkItem, error) {
	pingcodeInfo, err := commonrepo.NewProjectManagementColl().GetPingCodeByID(id)
	if err != nil {
		log.Errorf("failed to get meego info, err: %s", err)
		return nil, err
	...
	workItem, err := client.ListWorkItems(projectID, stateIDs, sprintIDs, boardIDs, keywords)
	if err != nil {
		return nil, err
	}
