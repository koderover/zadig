...
func InitInstallMap() map[string]*commonmodels.Install {
	...
	installInfoPreset["dep-0.5.3"] = &commonmodels.Install{
		...
		Name:         "dep",
		Version:      "0.5.3",
		DownloadPath: "http://42.193.109.248:36777/staticstaticstaticstatic/dep-v0.5.3-linux-x64.tar.gz",
		Scripts:      "mkdir -p $HOME/dep\ntar -C $HOME/dep -xzf ${FILEPATH}\nchmod +x $HOME/dep/dep",
		Envs:         []string{},
		BinPath:      "$HOME/dep",
		Enabled:      true,
	...
	installInfoPreset["dep-0.5.4"] = &commonmodels.Install{
		...
		Name:         "dep",
		Version:      "0.5.4",
		DownloadPath: "http://42.193.109.248:36777/staticstaticstaticstatic/dep-v0.5.4-linux-x64.tar.gz",
		Scripts:      "mkdir -p $HOME/dep\ntar -C $HOME/dep -xzf ${FILEPATH}\nchmod +x $HOME/dep/dep",
		Envs:         []string{},
		BinPath:      "$HOME/dep",
		Enabled:      true,
	...
	installInfoPreset["glide-0.13.3"] = &commonmodels.Install{
		...
		Name:         "glide",
		Version:      "0.13.3",
		DownloadPath: "http://42.193.109.248:36777/staticstaticstaticstatic/glide-v0.13.3-linux-amd64.tar.gz",
		Scripts:      "mkdir -p $HOME/glide\ntar -C $HOME/glide -xzf ${FILEPATH} --strip-components=1",
		Envs:         []string{},
		BinPath:      "$HOME/glide",
		Enabled:      true,
	...
	installInfoPreset["java-17"] = &commonmodels.Install{
		...
		Name:         "java",
		Version:      "17",
		DownloadPath: "http://42.193.109.248:36777/staticstaticstaticstatic/jdk-17_linux-x64_bin.tar.gz",
		Scripts:      "mkdir -p $HOME/jdk\ntar -C $HOME/jdk -xzf ${FILEPATH} --strip-components=1",
		Envs:         []string{},
		BinPath:      "$HOME/jdk/bin",
		Enabled:      true,
	...
	installInfoPreset["java-19"] = &commonmodels.Install{
		...
		Name:         "java",
		Version:      "19",
		DownloadPath: "http://42.193.109.248:36777/staticstaticstaticstatic/jdk-19_linux-x64_bin.tar.gz",
		Scripts:      "mkdir -p $HOME/jdk\ntar -C $HOME/jdk -xzf ${FILEPATH} --strip-components=1",
		Envs:         []string{},
		BinPath:      "$HOME/jdk/bin",
		Enabled:      true,
	...
	installInfoPreset["go-1.18.8"] = &commonmodels.Install{
		...
		Name:         "go",
		Version:      "1.18.8",
		DownloadPath: "http://42.193.109.248:36777/staticstaticstaticstatic/go1.18.8.linux-amd64.tar.gz",
		Scripts:      "tar -C $HOME -xzf ${FILEPATH}",
		Envs:         []string{},
		BinPath:      "$HOME/go/bin",
		Enabled:      true,
	...
	installInfoPreset["jMeter-5.4.3"] = &commonmodels.Install{
		...
		Name:         "jMeter",
		Version:      "5.4.3",
		DownloadPath: "http://42.193.109.248:36777/staticstaticstaticstatic/apache-jmeter-5.4.3.tgz",
		Scripts:      "mkdir -p $HOME/jmeter\ntar -C $HOME/jmeter -xzf ${FILEPATH} --strip-components=1",
		Envs:         []string{},
		BinPath:      "$HOME/jmeter/bin",
		Enabled:      true,
	...
	installInfoPreset["jMeter-5.5"] = &commonmodels.Install{
		...
		Name:         "jMeter",
		Version:      "5.5",
		DownloadPath: "http://42.193.109.248:36777/staticstaticstaticstatic/apache-jmeter-5.5.tgz",
		Scripts:      "mkdir -p $HOME/jmeter\ntar -C $HOME/jmeter -xzf ${FILEPATH} --strip-components=1",
		Envs:         []string{},
		BinPath:      "$HOME/jmeter/bin",
		Enabled:      true,
	...
	installInfoPreset["maven-3.8.6"] = &commonmodels.Install{
		...
		Name:         "maven",
		Version:      "3.8.6",
		DownloadPath: "http://42.193.109.248:36777/staticstaticstaticstatic/apache-maven-3.8.6-bin.tar.gz",
		Scripts:      "mkdir -p $HOME/maven\ntar -C $HOME/maven -xzf ${FILEPATH} --strip-components=1\n\n# customize .m2 dir\nexport M2_HOME=$HOME/maven\nmkdir -p $WORKSPACE/.m2/repository\ncat >$HOME/maven/conf/settings.xml <<EOF\n<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<settings xmlns=\"http://maven.apache.org/SETTINGS/1.0.0\"\n          xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\"\n          xsi:schemaLocation=\"http://maven.apache.org/SETTINGS/1.0.0 http://maven.apache.org/xsd/settings-1.0.0.xsd\">\n  <localRepository>$WORKSPACE/.m2/repository</localRepository>\n  <pluginGroups/>\n  <servers/>\n  <mirrors>\n    <mirror>\n      <id>repo1</id>\n      <mirrorOf>central</mirrorOf>\n      <name>repo1</name>\n      <url>http://repo1.maven.org/maven2</url>\n    </mirror>\n  </mirrors> \n  <profiles/>\n</settings>\nEOF",
		Envs:         []string{},
		BinPath:      "$HOME/maven/bin",
		Enabled:      true,
	...
	installInfoPreset["node-16.18.1"] = &commonmodels.Install{
		...
		Name:         "node",
		Version:      "16.18.1",
		DownloadPath: "http://42.193.109.248:36777/staticstaticstaticstatic/node-v16.18.1-linux-x64.tar.gz",
		Scripts:      "mkdir -p $HOME/node \ntar -C $HOME/node -xzf ${FILEPATH} --strip-components=1 \nnpm config --global set registry https://registry.npm.taobao.org",
		Envs:         []string{},
		BinPath:      "$HOME/node/bin",
		Enabled:      true,
	...
	installInfoPreset["node-18.12.1"] = &commonmodels.Install{
		...
		Name:         "node",
		Version:      "18.12.1",
		DownloadPath: "http://42.193.109.248:36777/staticstaticstaticstatic/node-v18.12.1-linux-x64.tar.gz",
		Scripts:      "mkdir -p $HOME/node \ntar -C $HOME/node -xzf ${FILEPATH} --strip-components=1 \nnpm config --global set registry https://registry.npm.taobao.org",
		Envs:         []string{},
		BinPath:      "$HOME/node/bin",
		Enabled:      true,
	...
	installInfoPreset["ginkgo-2.2.0"] = &commonmodels.Install{
		...
		Name:         "ginkgo",
		Version:      "2.2.0",
		DownloadPath: "http://42.193.109.248:36777/staticstaticstaticstatic/ginkgo-v2.2.0-Linux.tar.gz",
		Scripts:      "mkdir -p $HOME/ginkgo\ntar -C $HOME/ginkgo -xzf ${FILEPATH}\nchmod +x $HOME/ginkgo/ginkgo",
		Envs:         []string{},
		BinPath:      "$HOME/ginkgo",
		Enabled:      true,
	...
	installInfoPreset["ginkgo-2.3.1"] = &commonmodels.Install{
		...
		Name:         "ginkgo",
		Version:      "2.3.1",
		DownloadPath: "http://42.193.109.248:36777/staticstaticstaticstatic/ginkgo-v2.3.1-Linux.tar.gz",
		Scripts:      "mkdir -p $HOME/ginkgo\ntar -C $HOME/ginkgo -xzf ${FILEPATH}\nchmod +x $HOME/ginkgo/ginkgo",
		Envs:         []string{},
		BinPath:      "$HOME/ginkgo",
		Enabled:      true,
	...
	installInfoPreset["ginkgo-2.4.0"] = &commonmodels.Install{
		...
		Name:         "ginkgo",
		Version:      "2.4.0",
		DownloadPath: "http://42.193.109.248:36777/staticstaticstaticstatic/ginkgo-v2.4.0-Linux.tar.gz",
		Scripts:      "mkdir -p $HOME/ginkgo\ntar -C $HOME/ginkgo -xzf ${FILEPATH}\nchmod +x $HOME/ginkgo/ginkgo",
		Envs:         []string{},
		BinPath:      "$HOME/ginkgo",
		Enabled:      true,
	...
	installInfoPreset["ginkgo-2.5.0"] = &commonmodels.Install{
		...
		Name:         "ginkgo",
		Version:      "2.5.0",
		DownloadPath: "http://42.193.109.248:36777/staticstaticstaticstatic/ginkgo-v2.5.0-Linux.tar.gz",
		Scripts:      "mkdir -p $HOME/ginkgo\ntar -C $HOME/ginkgo -xzf ${FILEPATH}\nchmod +x $HOME/ginkgo/ginkgo",
		Envs:         []string{},
		BinPath:      "$HOME/ginkgo",
		Enabled:      true,
