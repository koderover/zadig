...
func waitAndGetDeployLog(ctx context.Context, streamChan chan interface{}, options *GetDeployJobLogOptions, log *zap.SugaredLogger) {
	...
	for {
		...
		default:
			...
			jobKey := deployJobLogManager.GetJobKey(options.ProjectKey, options.WorkflowKey, options.TaskID, options.JobName)
			jobLogKey := deployJobLogManager.GetJobLogKey(options.ProjectKey, options.WorkflowKey, options.TaskID, options.JobName)
