...
func ReleasePlanHookCallback(c *handler.Context, callback *ReleasePlanCallBackBody) error {
	...
	if callback.Result == setting.ReleasePlanCallBackResultTypeSuccess {
		...
		case config.ReleasePlanStatusWaitForSuccessExternalCheck:
			releasePlan.WaitForSuccessExternalCheckTime = time.Now().Unix()
		}
		...
		log.Debugf("release plan status: %s, hookEventStatusMap: %+v, nextStatus: %s", releasePlan.Status, hookEventStatusMap, nextStatus)
...
func sendReleasePlanHook(plan *models.ReleasePlan, hookSetting *commonmodels.ReleasePlanHookSettings) error {
...
func waitForExternalCheck(plan *models.ReleasePlan, hookSetting *commonmodels.ReleasePlanHookSettings) (*config.ReleasePlanStatus, bool) {
	shouldWait := false
	if !hookSetting.Enable {
