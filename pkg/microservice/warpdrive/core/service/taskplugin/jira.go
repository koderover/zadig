...
func (p *JiraPlugin) Run(ctx context.Context, pipelineTask *task.Task, pipelineCtx *task.PipelineCtx, serviceName string) {
	p.Task.Issues = make([]*task.JiraIssue, 0)

	jiraKeys := make([]string, 0)
...
func (p *JiraPlugin) getJiraIssue(pipelineTask *task.Task, key string) (*task.JiraIssue, error) {
	...
	if err != nil {
		fmt.Printf("GetIssueByKeyOrID [%s] error: %v user:%s access:%s host: %s,", key, err, jiraInfo.User, jiraInfo.AccessToken, jiraInfo.Host)
		return jiraIssue, fmt.Errorf("GetIssueByKeyOrID [%s] error: %v user:%s access:%s host: %s,", key, err, jiraInfo.User, jiraInfo.AccessToken, jiraInfo.Host)
	}
