...
func (c *PingCodeClient) ListWorkItems(projectID string, stateIDs, sprintIDs, boardIDs []string, keywords string) ([]*WorkItem, error) {
	...
	for {
		...
		if len(stateIDs) > 20 || len(sprintIDs) > 20 || len(boardIDs) > 20 {
			return nil, fmt.Errorf("stateIDs or sprintIDs or boardIDs count large than 20")
		}
		if len(stateIDs) != 0 {
