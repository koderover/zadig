...
func (c *OpenAIClient) Configure(config LLMConfig) error {
	...
	} else if strings.HasPrefix(string(config.GetProviderName()), string(ProviderAliyunBailian)) || strings.HasPrefix(string(config.GetProviderName()), string(ProviderVolcengineArk)) {
		c.apiType = string(openai.APITypeOpenAI)
		defaultConfig = openai.DefaultConfig(token)
		baseURL := config.GetBaseURL()
		defaultConfig.BaseURL = baseURL
