...
func (NetworkPolicyAnalyzer) Analyze(a Analyzer) ([]Result, error) {
	...
	for _, policy := range policies.Items {
				...
				Sensitive: []Sensitive{
					{
						...
						Masked:   MaskString(policy.Name),
					},
				},
			})
		} else {
			// Check if policy is not applied to any pods
			podList, err := GetPodListByLabels(a.Client.GetClient(), a.Namespace, policy.Spec.PodSelector.MatchLabels)
			if err != nil {
				return nil, err
			}
		...
		if len(failures) > 0 {
			preAnalysis[fmt.Sprintf("%s/%s", policy.Namespace, policy.Name)] = PreAnalysis{
