...
func DeleteReplicaSetsV2(ctx context.Context, clusterID, namespace string, opts ...DeleteOption) error {
	config := &deleteConfig{}
	...
	if config.selector == "" {
		return fmt.Errorf("must specify a selector for replica set deletion")
	}
	...
	err = c.DeleteAllOf(ctx, &appsv1.ReplicaSet{}, deleteOpts)
	return util.IgnoreNotFoundError(err)
}

func CreateOrPatchReplicaSetV2(ctx context.Context, clusterID, namespace, originalYAML, targetYAML string, resourceOverride bool) error {
	...
	var targetObj appsv1.ReplicaSet
	if err := json.Unmarshal(targetJSON, &targetObj); err != nil {
		return fmt.Errorf("failed to unmarshal target JSON to ReplicaSet: %w", err)
	}

	name := targetObj.GetName()
	if name == "" {
		return fmt.Errorf("replicaset name cannot be empty in target YAML")
	}
	...
	if originalYAML != "" {
		...
		var originalObj appsv1.ReplicaSet
	...
	_, err = c.AppsV1().ReplicaSets(namespace).Get(ctx, name, metav1.GetOptions{})
	if apierrors.IsNotFound(err) {
		_, createErr := c.AppsV1().ReplicaSets(namespace).Create(ctx, &targetObj, metav1.CreateOptions{})
		if createErr != nil {
			return fmt.Errorf("failed to create replicaset: %w", createErr)
		}
		return nil
	}
	if err != nil {
		return fmt.Errorf("failed to check replicaset existence: %w", err)
	}

	if resourceOverride {
		return retry.RetryOnConflict(retry.DefaultRetry, func() error {
			existing, err := c.AppsV1().ReplicaSets(namespace).Get(ctx, name, metav1.GetOptions{})
			if err != nil {
				return fmt.Errorf("failed to get replicaset for replace: %w", err)
			}
			targetObj.ResourceVersion = existing.ResourceVersion
			_, err = c.AppsV1().ReplicaSets(namespace).Update(ctx, &targetObj, metav1.UpdateOptions{})
			return err
		})
	}

	patchBytes, err := strategicpatch.CreateTwoWayMergePatch(originalJSONMutated, targetJSONMutated, &appsv1.ReplicaSet{})
	if err != nil {
	...
	_, err = c.AppsV1().ReplicaSets(namespace).Patch(
		ctx, name, types.StrategicMergePatchType, patchBytes, metav1.PatchOptions{},
	)
	if err != nil {
		return fmt.Errorf("replicaset patch failed: %w", err)
	}
