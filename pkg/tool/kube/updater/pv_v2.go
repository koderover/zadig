...
func DeletePersistentVolumesV2(ctx context.Context, clusterID string, opts ...DeleteOption) error {
	config := &deleteConfig{}
	...
	if config.selector == "" {
		return fmt.Errorf("must specify a selector for persistent volume deletion")
	}
	...
	err = c.DeleteAllOf(ctx, &corev1.PersistentVolume{}, deleteOpts)
